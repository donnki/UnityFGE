...
public class ByteBuffer
{
	...
	/**
	 * 从缓存区中读取指定长度的一个UTF8字符串
	 */
	public string ReadString(int length){
		byte[] _bytes = Read(length);
		flip(_bytes);
		return System.Text.Encoding.UTF8.GetString (_bytes);
	}

	/**
	 * 将字符串长度和UTF8字节数组写入至缓存区中
	 */
	public void WriteStringUshort(string str){
		byte[] bytes = System.Text.Encoding.UTF8.GetBytes(str);
	...
	/**
	 * 从缓存区中读取出长度后再读取相应长度的UTF8字符串
	 */
	public string ReadStringUShort(){
